...
public static class MeshGenerator
{
        ...
        int levelOfDetail)
    {
        ...
        int meshSimplificationInctrement = (levelOfDetail == 0) ? 1 : levelOfDetail * 2;
        int verticesPerLine = (width-1)/meshSimplificationInctrement+1;
        
        MeshData meshData = new MeshData(verticesPerLine, verticesPerLine);
        int vertexIndex = 0;

        for (int y = 0; y < height; y += meshSimplificationInctrement)
        {
            for (int x = 0; x < width; x += meshSimplificationInctrement)
            {
